...
const CAMPOS_BASE: { chave: CampoAlvo; label: string; obrigatorio: boolean; candidatos: string[] }[] = [
...
/** Definir o nicho de um lead (e, por consequência, criar quadro novo) é
 * ação de ADMIN — pra quem não é, a coluna/campo de nicho nem aparece
 * aqui, e o servidor ignora de qualquer forma se vier na requisição. */
export function ImportarLeadsForm({ isAdmin }: { isAdmin: boolean }) {
  const CAMPOS = useMemo(() => CAMPOS_BASE.filter((c) => isAdmin || c.chave !== "categoria"), [isAdmin]);
  ...
  async function importar() {
    ...
    try {
      const linhasMapeadas = linhas.map((linha) => ({
        ...
        categoria: isAdmin && mapeamento.categoria ? linha[mapeamento.categoria] : null,
      ...
      const res = await fetch("/api/leads/import", {
        ...
        body: JSON.stringify({
          ...
          tipo,
          origem,
          categoria: isAdmin ? categoria.trim() || null : null,
        }),
      });
      ...
      <div>
        ...
        {!isAdmin && (
          <p className="mt-1 text-xs text-ink-400">
            Esses leads entram em &quot;Sem nicho definido&quot; — só um administrador organiza por
            nicho.
          </p>
        )}
      </div>
